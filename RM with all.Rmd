...
t.test(mean_phase1, mu = .5, alternative = "greater") #p-value = 2.317e-10

mean_phase2 <- phase2 %>%
  group_by(pNum) %>%
   summarise(mean_response = mean(prob_response, na.rm = TRUE))
t.test(mean_phase2, mu = .5, alternative = "greater") #p-value = 3.183e-10

mean_cert_phase3 <- filter(phase3, cue_type == "certain") %>%
  group_by(pNum) %>%
   summarise(mean_response = mean(prob_response, na.rm = TRUE))
t.test(mean_cert_phase3, mu = .5, alternative = "greater") #p-value = 2.056e-10

mean_uncert_phase3 <- filter(phase3, cue_type == "uncertain") %>%
  group_by(pNum) %>%
   summarise(mean_response = mean(prob_response, na.rm = TRUE))
t.test(mean_uncert_phase3, mu = .5, alternative = "greater") #p-value = 2.661e-10

#ANOVAS with the exp effect
#prepare ANOVA phase 1
exp_response_phase1 <- phase1 %>%
  group_by (exp_code, pNum, block) %>%
  summarise(mean_response = mean(prob_response, na.rm = TRUE))
exp_response_phase1$exp_code <- factor(exp_response_phase1$exp_code)
exp_response_phase1$block <- factor(exp_response_phase1$block)
exp_response_phase1$pNum <- factor(exp_response_phase1$pNum)

#prepare ANOVA phase 2
exp_response_phase2 <- phase2 %>%
  group_by (exp_code, pNum, block) %>%
  summarise(mean_response = mean(prob_response, na.rm = TRUE))
exp_response_phase2$exp_code <- factor(exp_response_phase2$exp_code)
exp_response_phase2$block <- factor(exp_response_phase2$block)
exp_response_phase2$pNum <- factor(exp_response_phase2$pNum)
...
# ANOVA 1 with exp_code
exp_ANOVA_p1_resp <-  aov_car(formula = mean_response ~ exp_code + Error(pNum/block), data = exp_response_phase1)
exp_bay_ANOVA_p1_resp <- anovaBF(formula = mean_response ~ block*exp_code + pNum,
        data = data.frame(exp_response_phase1),
        whichRandom = "pNum")
exp_bay_ANOVA_p1_resp [4] / exp_bay_ANOVA_p1_resp [3]

#ANOVA 2 with exp_code
exp_ANOVA_p2_resp <- aov_car(formula = mean_response ~ exp_code + Error(pNum/block), data = exp_response_phase2)
exp_bay_ANOVA_p2_resp <- anovaBF(formula = mean_response ~ block*exp_code + pNum,
        data = data.frame(exp_response_phase2),
        whichRandom = "pNum")
exp_bay_ANOVA_p2_resp [4] / exp_bay_ANOVA_p2_resp [3]
...
exp_bay_ANOVA_p3_resp <- anovaBF(formula = mean_response ~ block*cue_type*exp_code + pNum,
        ...
        whichRandom = "pNum")
exp_bay_ANOVA_p3_resp [4] / exp_bay_ANOVA_p3_resp [3]
exp_bay_ANOVA_p3_resp [10] / exp_bay_ANOVA_p3_resp [6]
exp_bay_ANOVA_p3_resp [18] / exp_bay_ANOVA_p3_resp [17]
#it prints a warning message that participant 1 and 7 have been removed because some cells are missing
#let's modify the data so we can perform the analysis with chunkcs of 2 blocks
corrected_phase3 <-  mutate(phase3, 
               twoblocks = case_when(block == 5 | block == 6 ~ 1,
                                    ...
                                    block == 15 | block == 16 ~ 6,))
#now anova on that
exp_c_response_phase3 <- corrected_phase3 %>%
  group_by (exp_code, pNum, twoblocks, cue_type) %>%
  summarise(mean_response = mean(prob_response, na.rm = TRUE))
exp_c_response_phase3$exp_code <- factor(exp_c_response_phase3$exp_code)
exp_c_response_phase3$twoblocks <- factor(exp_c_response_phase3$twoblocks)
exp_c_response_phase3$cue_type <- factor(exp_c_response_phase3$cue_type)
exp_c_response_phase3$pNum <- factor(exp_c_response_phase3$pNum)
exp_c_ANOVA_p3_resp <- aov_car(formula = mean_response ~ exp_code + Error(pNum/twoblocks*cue_type), data = exp_c_response_phase3)
exp_c_bay_ANOVA_p3_resp <- anovaBF(formula = mean_response ~ twoblocks*cue_type*exp_code + pNum,
        data = data.frame(exp_c_response_phase3),
        whichRandom = "pNum")
# Calculate interaction Bayes Factor
exp_c_bay_ANOVA_p3_resp[4] / exp_c_bay_ANOVA_p3_resp[3]
exp_c_bay_ANOVA_p3_resp[10] / exp_c_bay_ANOVA_p3_resp[6]
exp_c_bay_ANOVA_p3_resp[18] / exp_c_bay_ANOVA_p3_resp[17]
```

In phase 1, nor the main effect of the experiment or it's interaction with the block were significant (`r apa(exp_ANOVA_p1_resp, effect = "exp_code", format = "rmarkdown")`, `r apa(exp_ANOVA_p1_resp, effect = "exp_code:block", format = "rmarkdown")`). For both, anecdotal evidence for the null hypothesis was found (*BF* = 0.3483414 ±0.91%, *BF* = 0.3277184 ±2.62%). Similar results were found in phase 2 (`r apa(exp_ANOVA_p2_resp, effect = "exp_code", format = "rmarkdown")`, `r apa(exp_ANOVA_p2_resp, effect = "exp_code:block", format = "rmarkdown")`), with still anecdotal evidence against the effect of the experiment (*BF* = 0.3355596 ±1.02%), but anecdotal evidence in favor of the effect of the interaction of the experiment and block. Again, in phase 3, there were no significant effect of the experiment or any of the interactions with that effect (Exp: `r apa(exp_c_ANOVA_p3_resp, effect = "exp_code", format = "rmarkdown")`, ExpxBlocks: `r apa(exp_c_ANOVA_p3_resp, effect = "exp_code:twoblocks", format = "rmarkdown")`, ExpxCue: `r apa(exp_c_ANOVA_p3_resp, effect = "exp_code:cue_type", format = "rmarkdown")`, ExpxBlocksxCue `r apa(exp_c_ANOVA_p3_resp, effect = "exp_code:twoblocks:cue_type", format = "rmarkdown")`). Bayesian analysis yielded anecdotal evidence against the main effect (*BF* = 0.3491084 ±0.79%), moderate null evidence for the ExpxCue and ExpXBlocks interactions (*BF* = 0.2307687 ±7.63%, *BF* = 0.1490516 ±1.47%), and very strong evidence for the null hypothesis in the ExpxBlocksxCue interaction (*BF*= 0.06828904 ±3.2%). Given that the effect of the experiment did not seem to have a significant effect on the results, they were re-analysed without including the experiment as a factor.
...
response_phase1 <- phase1 %>%
  group_by (exp_code, pNum, block) %>%
  summarise(mean_response = mean(prob_response, na.rm = TRUE))
response_phase1$exp_code <- factor(response_phase1$exp_code)
...
response_phase2 <- phase2 %>%
  group_by (exp_code, pNum, block) %>%
  summarise(mean_response = mean(prob_response, na.rm = TRUE))
response_phase2$exp_code <- factor(response_phase2$exp_code)
...
A within-subject ANOVA for phase 1 showed a significant effect of the block (`r apa(ANOVA_p1_resp, effect = "block", format = "rmarkdown")`), for which very strong evidence was found (*BF* = 43.84792 ±1.13%). This indicated that the accuracy to certain cues increased from block 1 to block 2. However, in phase 2, there was not a significant effect of the block (`r apa(ANOVA_p2_resp, effect = "block", format = "rmarkdown")`), finding anecdotal evidence for the null hypothesis (*BF* = 0.7254381 ±0.97%), indicating that participants did not improved their accuracy in phase 2. Regarding phase 3, both the main effect of the type of cue and the blocks were significant (Blocks: `r apa(c_ANOVA_p3_resp, effect = "twoblocks", format = "rmarkdown")`, Cue type: `r apa(c_ANOVA_p3_resp, effect = "cue_type", format = "rmarkdown")`), as well as the interaction between them (`r apa(c_ANOVA_p3_resp, effect = "twoblocks:cue_type", format = "rmarkdown")`). There was extreme strong evidence for both main effects (*BF* = 496.378 ±0.52%, *BF* = 893649670 ±0.84%), but there was moderate evidence for the null hypothesis in the interaction (*BF* = 0.1988564 ±2%). This indicates that all subjects improved their performance as training progressed, and that accuracy was better for certain than uncertain cues, especially at the end of the experiment.
